...
void reflow(float temperature) {
    ...
    } else if (((temperature > selectedProfile.ramp.temp) && (temperature < selectedProfile.soak.temp)) && ((seconds > selectedProfile.ramp.second) && (seconds < selectedProfile.soak.second))) {
    ...
    } else if (((temperature > selectedProfile.soak.temp) && (temperature < selectedProfile.reflow.temp)) && ((seconds > selectedProfile.soak.second) && (seconds < selectedProfile.reflow.second))) {
    ...
    } else if ((temperature > selectedProfile.reflow.temp) || ((seconds > selectedProfile.reflow.second))) {
...
void loop() {
    ...
    delay(500);
    millis_now = millis();
    if ((float)(millis_now - millis_before_2) > pid_refresh_rate) { // Refresh rate of the PID
    ...
    millis_now = millis();
    if ((float)(millis_now - millis_before) > refresh_rate) { // Refresh rate of printing on the LCD
        refreshDisplay();
    }
